...
def load_employee_faces(base_path):
    ...
    employee_faces_path = f"{base_path}/employee_faces".replace("/", "\\")
    print(employee_faces_path)
    print(os.path.exists(employee_faces_path))
...
#base_path = "C:/Users/DY/Downloads/Compressed/capstone-bsit--main_2/capstone-bsit--main/admin_area/"
if os.path.exists(config_file_name):
    with open(config_file_name) as conf_file:
        base_path = conf_file.readlines()[-1]
else:
    base_path = ""
...
def schedule_daily_tasks():
    schedule.every().day.at("17:00").do(Dq.mark_absent_employees)
    schedule.every().day.at("23:59").do(Dq.transfer_attendance_to_report)
        ...
        if ret:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if face_distances[best_match_index] < 0.6: 
                    ...
                    self.emp_id_disp.configure(text = f"ID: Unknown")
                    self.emp_name_disp.configure(text = f"Name: ")
                    self.emp_status_disp.configure(text = f"Status:")
    ...
    def open_config(self):
        ...
        def get_img_path():
            ...
            if selected_folder:
                ...
                config_input[img_path_txt].configure(state="normal")
                config_input[img_path_txt].insert(0, self.folder_path)
        ...
        def save_config():
            ...
            config += config_input[img_path_txt].get()
        ...
        if os.path.exists(config_file_name):
            with open(config_file_name, "r") as config_file:
                ...
                for pos, conf in enumerate(config_param):
                    config_input[f"{conf}_input"].insert(0, configuration[pos])
                img_path = -1 if len(configuration[-1]) > 0 else -2
                config_input[img_path_txt].configure(state="normal")
                config_input[img_path_txt].insert(0, configuration[img_path])
                config_input[img_path_txt].configure(state="disabled")
